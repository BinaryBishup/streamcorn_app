                ...
                return (
                  <button key={p.id} onClick={() => switchProfile(p)} className={`w-full flex items-center gap-3 px-3 py-2.5 active:bg-white/[0.06] ${p.id === activeProfile?.id ? 'bg-white/[0.04]' : ''}`}>
                    <div className="w-7 h-7 rounded-md overflow-hidden bg-[#333] flex-shrink-0">
                      {pImg ? <Image src={p.avatar_url!} alt="" width={28} height={28} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: '#e50914' }}>{p.name.charAt(0)}</div>}
                    </div>
