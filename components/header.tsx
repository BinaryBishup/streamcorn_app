    ...
    <header className="fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-4 h-12 bg-gradient-to-b from-black/90 to-transparent">
      ...
      <div className="flex items-center gap-2">
        <Link href="/search" className="w-9 h-9 flex items-center justify-center">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth={2} className="drop-shadow"><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
        </Link>
        ...
        <div className="relative" ref={dropdownRef}>
          <button onClick={() => setShowDropdown(!showDropdown)} className="w-8 h-8 rounded-lg overflow-hidden bg-[#e50914] flex items-center justify-center shadow-lg">
            {isImage ? (
              ...
              <span className="text-white text-sm font-bold">{activeProfile?.name?.charAt(0).toUpperCase() || 'U'}</span>
            )}
          </button>
                ...
                return (
                  <button key={p.id} onClick={() => switchProfile(p)} className={`w-full flex items-center gap-3 px-3 py-2.5 active:bg-white/[0.06] ${p.id === activeProfile?.id ? 'bg-white/[0.04]' : ''}`}>
                    <div className="w-7 h-7 rounded-md overflow-hidden bg-[#333] flex-shrink-0">
                      {pImg ? <Image src={p.avatar_url!} alt="" width={28} height={28} className="w-full h-full object-cover" /> : (
                        <div className="w-full h-full flex items-center justify-center text-white text-xs font-bold" style={{ backgroundColor: p.avatar_url?.startsWith('#') ? p.avatar_url : '#e50914' }}>{p.name.charAt(0)}</div>
                      )}
                    </div>
