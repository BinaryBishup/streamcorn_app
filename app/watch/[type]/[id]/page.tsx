...
export default function WatchPage() {
  ...
  // ─── Autoplay (muted) + resume position ─────────────────────────
  useEffect(() => {
    const v = videoRef.current
    if (!v || !ready) return
    ...
    const onLoaded = () => {
      ...
      setMuted(true)
      v.play().catch(() => {
        // If even muted play is rejected, the first user tap on the
        // stage will retry via the `enterImmersive` handler below.
      })
  ...
  // ─── Immersive: fullscreen + landscape lock on first interaction ─
  const enterImmersive = useCallback(async () => {
    ...
    // Unmute + (re)play on this same gesture so sound kicks in.
    v.muted = false
    setMuted(false)
    v.play().catch(() => {})
  }, [])
    ...
    >
      ...
      {muted && (
        <button
          ...
          className="absolute top-20 right-4 z-10 flex items-center gap-1.5 px-3 py-2 rounded-full bg-black/70 backdrop-blur text-white text-xs font-semibold active:bg-black/85"
          aria-label="Unmute"
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M16.5 12c0-1.77-1-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63m2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51A8.8 8.8 0 0 0 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71M4.27 3 3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06a8.99 8.99 0 0 0 3.69-1.81L19.73 21 21 19.73l-9-9zM12 4 9.91 6.09 12 8.18z"/></svg>
          Tap to unmute
        </button>
      )}
