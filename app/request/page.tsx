...
interface ContentRequest {
  ...
  poster_path: string | null; status: string; vote_count: number
...
export default function RequestPage() {
  ...
  const otherRequests = requests.filter(r => r.user_id !== userId)
  ...
  return (
    <div className="min-h-screen bg-black pt-4 px-4 pb-20">
                ...
                <div className="self-center flex-shrink-0">
                  {alreadyRequested ? (
                    ...
                    <button onClick={() => submitRequest(item)} disabled={requestingId === item.id}
                      className="px-3.5 py-1.5 bg-[#e50914] text-white text-[11px] font-bold rounded-lg active:bg-[#b20710] disabled:opacity-50">
              ...
              return (
                <div key={req.id} className="flex gap-3 p-3 bg-[#111] rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/40 text-[11px] capitalize">{req.type}</p>
                  </div>
                  <div className="self-center flex-shrink-0">
      ...
      {/* Community Requests */}
      {!loadingRequests && otherRequests.length > 0 && (
        <div className="mb-6">
          <h2 className="text-white font-semibold text-sm mb-3">Community Requests</h2>
          <div className="space-y-2">
            {otherRequests.map(req => {
              ...
              return (
                <div key={req.id} className="flex gap-3 p-3 bg-[#111] rounded-xl">
                  ...
                  <div className="self-center flex-shrink-0 text-right">
                    <span className={`${s.bg} ${s.text} text-[10px] font-semibold px-2.5 py-1 rounded-full`}>{s.label}</span>
                    {req.vote_count > 1 && <p className="text-white/25 text-[10px] mt-1">{req.vote_count} votes</p>}
                  </div>
                </div>
              )
            })}
