...
// Genre chips — sourced to match what the live catalogue actually uses
...
export default function BrowsePage() {
  ...
  useEffect(() => { setPage(1); fetchContent(1, false) }, [type, genre, fetchContent])
  ...
  const activeFilters = [type, genre].filter(Boolean).length
    ...
    <div className="min-h-screen bg-black">
      ...
      {activeFilters > 0 && (
        <div className="flex items-center gap-2 px-4 py-2">
          ...
          <button onClick={() => { setType(null); setGenre(null) }} className="text-white/30 text-[11px] ml-auto">Clear</button>
        </div>
      )}
      ...
      <div ref={contentRef}>
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={() => { setType(null); setGenre(null) }} className="mt-3 text-[#e50914] text-sm font-medium">Clear filters</button>
          </div>
        ) : (
          <div className="grid grid-cols-3 gap-2 px-4">
            {items.map(item => (
